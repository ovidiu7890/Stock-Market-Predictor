"""
Training loop for the Multi-Agent Cross-Attention Ensemble.

Usage::

    python -m pytorch_agents.train
    python -m pytorch_agents.train --config src/pytorch_agents/configs/default.yaml
    python -m pytorch_agents.train --epochs 5 --subset 50
"""
...
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # Stock-Market-Predictor/
...
# ======================================================================
# Helper: re-train the sklearn models in-memory (mirrors the notebooks)
# ======================================================================

def _train_chartist(cfg: dict):
    """Re-trains the Random Forest model exactly as in technical_agent.ipynb."""
    path = PROJECT_ROOT / cfg["paths"]["technical_data"]
...
def _train_newsroom(cfg: dict):
    """Re-trains the SVM + TfidfVectorizer exactly as in Sentiment_Agent.ipynb."""
...
def train(cfg: dict, epochs: int | None = None, subset: int | None = None):
    ...
    # Time-ordered split (no shuffle for time-series)
    split_idx = int(len(tech_df) * (1 - cfg["training"]["val_split"]))
    train_tech = tech_df.iloc[:split_idx]
    val_tech = tech_df.iloc[split_idx - seq_len:]  # overlap by seq_len for window continuity
    ...
    class_weights = class_weights / class_weights.sum() * 3  # normalise to sum to n_classes
